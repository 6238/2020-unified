...
public class Robot extends TimedRobot {
    ...
    public void robotInit() {
        ...
        SmartDashboard.putNumber("Shooter Input RPM", 0);
    ...
    public void teleopShooter() {
        ...
        if((p != kP)) { leftController.setP(p); kP = p; }
        if((i != kI)) { leftController.setI(i); kI = i; }
        if((d != kD)) { leftController.setD(d); kD = d; }
        if((iz != kIz)) { leftController.setIZone(iz); kIz = iz; }
        if((ff != kFF)) { leftController.setFF(ff); kFF = ff; }
        if((max != kMaxOutput) || (min != kMinOutput)) { 
            leftController.setOutputRange(min, max); 
            kMinOutput = min; kMaxOutput = max; 
        }

        double setPoint = SmartDashboard.getNumber("Shooter Input RPM", 0);
        leftController.setReference(setPoint, ControlType.kVelocity);
        
        SmartDashboard.putNumber("Shooter Actual RPM", leftEncoder.getVelocity());
    }
    /**
